...
### 7.6 Matiz importante: "mejor discriminación" no es lo mismo que "más ciclos confiables"

Un hallazgo adicional, identificado después de integrar la alerta de confiabilidad (sección 10): aunque la segmentación v2 no mejoró la capacidad real de distinguir Sano de Soplo (AUC casi idéntico, ver 7.4), sí detectaba muchos más ciclos por grabación que v1 (mediana de 28 contra ~3 en CirCor). Esto importa en la práctica: con v1, una fracción considerable de grabaciones reales dispara la alerta `"confiable": false` por detectar muy pocos ciclos (ver ejemplo de `9979_AV.wav`, sección 10). Con v2, esa situación sería mucho menos frecuente.

Son dos beneficios distintos que no se evaluaron por separado: (a) precisión de clasificación, y (b) frecuencia con la que el sistema puede dar una respuesta que valga la pena confiar. v1 gana en (a); v2 probablemente hubiera ganado en (b), aunque esto no se midió directamente. Queda como pregunta abierta si **combinar la segmentación v2 con el modelo v1** (usar v2 solo para generar más pedazos de audio, pero clasificar con el modelo ya entrenado en v1) daría lo mejor de ambos — es una combinación no probada, y no debe asumirse que funciona bien solo porque cada pieza funciona bien por separado; requeriría su propia validación honesta antes de usarse.

## 8. Cómo se procesa un audio nuevo en producción (estado actual)

Para que quede documentado con claridad qué le pasa exactamente a una grabación cuando alguien la sube a través de la aplicación:

1. **Preprocesamiento**: `preprocess_audio` carga el audio y lo normaliza (divide entre su valor máximo absoluto). **No se aplica ningún filtro de ruido** (ni pasa-banda, ni de ningún otro tipo) — esta es una limitación conocida, documentada como oportunidad de mejora (ver recomendación en sección 11).
2. **Segmentación**: se usa `detect_cycles` — el pipeline **v1 original**, sin cambios. La segmentación v2 explorada en la sección 7 **no está en producción**.
3. **Extracción de features**: `extract_features` calcula MFCCs (13 coeficientes) + RMS por cada ciclo detectado.
4. **Selección de modelo según edad**: si se recibe una edad ≥18 años, se usa el modelo de adultos (`modelo_pcg_soplo_adultos_rf.joblib`, entrenado con PhysioNet 2016, filtrado por calidad SQI). Si la edad es menor a 18, o no se proporciona, se usa el modelo pediátrico (`modelo_pcg_soplo_rf.joblib`, entrenado con CirCor). **Aún no está conectada la fuente real de la edad del paciente desde el resto de la aplicación** — pendiente de integrar con `server.js` / el formulario de carga.
5. **Clasificación y voto por mayoría**: cada ciclo recibe una predicción (Sano/Soplo) según el umbral del modelo elegido; la clase final de la grabación es la que gane por mayoría de votos entre sus ciclos.
6. **Alerta de confiabilidad**: si se detectaron menos de 5 ciclos, la respuesta incluye `"confiable": false` y una advertencia explícita, para no presentar un porcentaje de confianza engañoso.

## 9. Validación en población adulta (fuera de la muestra de entrenamiento)

Dado que la aplicación está pensada para población general (no solo pediátrica), y que el 92% de los pacientes de CirCor son niños/adolescentes (988 niños, 311 infantes, 127 adolescentes, 11 neonatos, de un total de 1568 — fuente: *The CirCor DigiScope Dataset: From Murmur Detection to Murmur Classification*, arXiv:2108.00813), se evaluó el modelo de producción actual contra una fuente de adultos: el **PhysioNet/CinC Challenge 2016** (subconjunto `training-a`, 409 grabaciones, explícitamente enfocado en corazones de adultos).

**Resultado:**

| Métrica | Pacientes pediátricos (CirCor, examen final) | Pacientes adultos (PhysioNet 2016, training-a) |
|---|---|---|
| Sensibilidad | 0.735 | 0.887 |
| Especificidad | 0.699 | **0.026** |
| Índice de Youden (J) | 0.434 | **−0.087** |

El modelo etiqueta como "Soplo" a prácticamente cualquier grabación de adulto, sin importar si en realidad está sana (solo 3 de 117 personas sanas fueron clasificadas correctamente). El índice de Youden negativo confirma que, en población adulta, el modelo se comporta **peor que el azar** — no es una degradación parcial, es una falta total de generalización a esta población.

**Conclusión:** el modelo actual **no debe usarse con pacientes adultos** sin antes reentrenar o recalibrar con datos de esa población. Esto es consistente con lo esperado dado que el modelo nunca vio un adulto durante su entrenamiento — la advertencia sobre este hueco (sección 6) queda confirmada empíricamente, no solo como sospecha teórica.

## 10. Alerta de confiabilidad implementada en producción

Se agregó a `arboldeprediccion.py` una señal explícita de baja confiabilidad (`"confiable": false` en la respuesta JSON) cuando se detectan menos de 5 ciclos cardiacos en una grabación — el mínimo usado como criterio de calidad durante el entrenamiento. Esto evita que el sistema reporte un "100% de confianza" engañoso en casos donde, por ejemplo, la segmentación solo logró aislar 1 ciclo de una grabación completa.

## 11. Recomendaciones finales, actualizadas

1. **Prioridad alta: conseguir e integrar datos de adultos antes de usar el sistema con población general.** PhysioNet 2016 ya fue identificado y probado como fuente viable — el siguiente paso sería entrenar (no solo evaluar) con esa fuente, con cuidado de no mezclarla ciegamente con CirCor por el riesgo de que el modelo aprenda a distinguir el origen de la grabación en vez de la condición médica (ver sección 7.4 para la lección aprendida al respecto). Alternativas: modelos separados por grupo de edad, o un modelo combinado con validación cuidadosa de que no esté aprendiendo por "atajo de origen".
2. Buscar o generar más ejemplos de la clase Click — ninguna fuente pública consultada la tiene etiquetada en volumen.
3. Si se retoma el trabajo de segmentación, calibrar parámetros usando solo el conjunto `dev`, nunca mirando las grabaciones de prueba final (lección de la sección 7.4).
4. Explorar reducción de ruido por sustracción espectral o filtro de Wiener como preprocesamiento adicional — más viable que una red neuronal de "denoising", dado que no se cuenta con pares de audio limpio/ruidoso de las mismas grabaciones para entrenar ese tipo de modelo.
